...
function setReaction(sender, reaction, postId, newReaction) {
    ...
    if (reaction == "") {
        ...
        likes.setAttribute('onclick', `onPostLikeClick(this, '${newReaction.toString()}', ${postId})`);
        dislikes.setAttribute('onclick', `onPostDislikeClick(this, '${newReaction.toString()}', ${postId})`);
    }
    else {
        if (reaction.toLowerCase() == newReaction.toString()) {
            removeReaction(sender, postId)
            likes.setAttribute('onclick', `onPostLikeClick(this, '', ${postId})`);
            dislikes.setAttribute('onclick', `onPostDislikeClick(this, '', ${postId})`);
        }
        else {
            putReaction(sender, newReaction, postId);
            likes.setAttribute('onclick', `onPostLikeClick(this, '${newReaction.toString()}', ${postId})`);
...
function putReaction(sender, reaction, postId) {
    $.ajax({
        ...
        success: function (result) {
            ...
            if (reacted != null) reacted.classList.remove("reacted");
...
function setChoice(target, isLiked) {
    $.ajax({
        type: "POST",
        url: `/profile/postChoice?target=${target}&isLiked=${isLiked}`,
        error: function (jqXHR, textStatus, error) {
            console.log(textStatus);
        }
    });
...
function createChat(target) {
    $.ajax({
        type: "POST",
        url: `/chat/createPrivateChat?companionUsername=${target}`,
        success: function (result) {
